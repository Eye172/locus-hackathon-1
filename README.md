...
- **Любой вуз по запросу**: офлайн-индекс 14 467 вузов → живой поиск Wikidata → OpenStreetMap (Nominatim) → Google Карты (serper.dev) для вузов, которых нет ни в Wikidata, ни в OSM: название, координаты, город, сайт; профиль собирается так же (кандидат помечен «веб»). Так находятся филиалы: «Cardiff University Astana» — это Cardiff University Kazakhstan на Тәуелсіздік, а не Кардифф в Уэльсе. Два правила против уверенной ошибки: если в запросе названо место, а кандидат из индекса стоит в другой стране, он перестаёт считаться точным совпадением — поиск не останавливается и доходит до веба; а филиалу подставляется его собственный сайт в национальной зоне (`cardiff.edu.kz`), иначе профиль собрался бы по фотографиям головного кампуса в другой стране.
...
- **CampusLense сам ищет в соцсетях — для любого вуза мира**: пользователь только выбирает вуз, запросы в TikTok («Топ», и оттуда же слайдшоу со вкладки «Фото») и Instagram («Популярное» и хэштег вуза) отправляет сервер. Запросов много и они про место, а не про объявления: название и сокращение вуза, плюс «campus», «student life», «dorm», «atmosphere», «campus tour», «lecture hall», «assembly hall», «library», «laboratory», «gym», «canteen» — по-английски для всех и ещё раз на языке, на котором пишут его студенты: «КБТУ общежитие», «Technische Universität München Wohnheim», «東京大学 キャンパス», «清华大学 校园». Язык определяется по стране вуза (`backend/app/pipeline/langs.py`, 29 языков, для остальных стран — английский), название на этом языке берётся из Wikidata, сокращение — то, чьи заглавные буквы совпадают с инициалами нынешнего названия (КазНУ, а не советское КазГУ). Первый профиль получает название и два самых отдачных запроса на каждом языке, второй проход — все. Google Картинки тоже повторяют поиск на местном языке. Всё найденное — только кандидаты: по «Nazarbayev University» приходят и посты о Назарбаев Интеллектуальных школах, поэтому фото показывается, только если ИИ узнал сам кампус. Рилсы и ролики открываются и дают кадры из середины.
- **Ничего не готовится заранее**: профиль собирается в тот момент, когда пользователь ввёл название. Если этот вуз
  открывали раньше, прошлый сбор показывается сразу — но помечен как прошлый, и живой сбор всё равно идёт и заменяет
  его. Прогрев (`scripts/prewarm.py`) ускоряет только первый экран, ответом он не становится.
- **Два прохода**: первый укладывается в требование кейса — полезный профиль за ≤ 25 с. После него сбор не
  обрывается по времени: источники, не успевшие к первому профилю, дорабатывают, недокачанные картинки докачиваются,
  соцсети проходятся полностью (все запросы на обоих языках, больше страниц, кадры из десяти роликов на источник, вся
  лента вуза и все посты с его отметкой), упавший источник пробуется ещё раз, контур кампуса запрашивается повторно,
  если Overpass не успел, а ИИ-инспектор досматривает всё остальное. Фоновый проход молчит: в открытой странице
  остаётся «Профиль собран за N с», а профиль тихо приходит заново раз в ~20 с, пока есть новое. Лимита по времени нет —
  только страховка от зависания (`background_guard_s`, 15 мин). Если закрыть страницу после первого профиля, сборка
  доработает и сохранит результат, а открытая заново страница подключится к ней, а не запустит вторую. КазНИТУ им.
  Сатпаева: 7 фото в первом профиле на 22-й секунде, 127 фото (420 изображений, 280 проверено ИИ) — на 71-й.
- **Фильтр рассчитан на широкий поиск**: кандидатов из соцсетей и картинок — сотни, а у ИИ-инспектора ограничены время и суточный лимит запросов. Поэтому он работает как очередь с приоритетом, а не «кто первый пришёл»: первыми смотрятся кадры, которые CLIP считает фотографиями (а не афишей, картой или скриншотом), которые похожи на эталонный снимок вуза и которые без его вердикта не будут показаны вовсе (находки поиска). Недосмотренное к концу первого прохода не теряется, а ждёт второго. Каждый источник отдаёт то, что успел найти к своему сроку, а не «всё или ничего». Отдельная проверка на сток (`backend/app/pipeline/crossdup.py`): если та же картинка (pHash, отличие ≤ 6 из 64 бит) уже есть в профиле вуза дальше 30 км, находка поиска отклоняется — это стоковое фото или чужой кампус. Снимки с собственных каналов вуза эта проверка не трогает.
- **Кадр вместо обложки**: обложка ролика в TikTok — это постер: заголовок крупными буквами, лицо, стрелка. Инспектор справедливо её отклоняет, хотя за ней часто лежит единственная существующая съёмка кампуса — атриум с пальмами, коридор между парами, очередь в столовой. Поэтому ролик скачивается и ffmpeg берёт кадры на трети и двух третях (`backend/app/pipeline/video_frames.py`); кадры кэшируются по id видео, поэтому три источника TikTok качают один ролик один раз, а повторная сборка — ни одного. Без ffmpeg источники честно откатываются на обложки.
...
# ffmpeg (опционально): кадры из середины видео TikTok; без него берутся обложки
...
python scripts/prewarm.py                   # опционально: греет кэш для первого экрана; ответ всё равно собирается заново
...
- **Суточный лимит ИИ-инспектора.** Бесплатный ключ Gemini даёт 500 запросов в сутки на модель. Когда лимит основной модели (`gemini-3.5-flash-lite`) кончается, инспектор сам переходит на `gemini-3.1-flash-lite-preview`, затем на `gemini-3.1-flash-lite` — у каждой свой лимит. Запасные модели измерены на тех же размеченных фото: точность 0.86 и 0.82 против 0.90 у основной, полнота 0.78 и 0.69 против 0.93 — они ошибаются в сторону «отклонить», то есть профиль беднее, но не врёт. Какая модель проверяла, видно в полосе агентов. Если исчерпаны все три, фото из поиска не показываются вовсе, и профиль говорит об этом прямо. С платным ключом лимита нет, стоимость ≈ 1–2 цента на вуз.
...
- Презентация (8 слайдов): `docs/pitch/CampusLense.pdf` (генератор `docs/pitch/build_deck.mjs`, слайд «Команда» — заполнить).
