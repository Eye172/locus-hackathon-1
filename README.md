...
| Агент | Что делает |
|---|---|
| **Scout** | ищет кандидатов там, где принадлежность вузу гарантирована источником: официальный сайт (и его соцсети: Telegram-канал, YouTube-канал, VK по ключу; Instagram/Facebook/TikTok закрыты без входа и даются ссылками), категория и «depicts» Wikimedia Commons, статьи Википедии, геофото внутри полигона кампуса из OpenStreetMap, Mapillary / Flickr / Google Places (по ключам) |
...
- **Ступени приближения** (грузится только то, на что смотрят):
  1. планета — пузырьки стран с числом вузов (`geo/countries.json`, несколько КБ);
  2. страна — пузырьки городов (`geo/c/{ISO}.json` грузится при входе);
  3. город — значки вузов с плоскими карточками;
  4. кампус — один вуз, отдельный режим.
  Вниз — двойной клик, вверх — прокрутка за нижний предел или «хлебные крошки». На каждой ступени свои слои карты,
  пределы зума и наклона; скорость колеса и вращения ограничена; на компьютерах без видеокарты включается лёгкий режим.
  Данные собирает `python scripts/build_geo_levels.py` (группы по городам, синонимы названий, дозаполнение координат OSM).
...
- **Соцсети как источник фото**: ссылки берутся с официального сайта; Telegram (посты с фото и подписями), YouTube (кадры видео), VK (по бесплатному сервисному токену `VK_SERVICE_TOKEN`). У каждого фото ссылка на пост.
