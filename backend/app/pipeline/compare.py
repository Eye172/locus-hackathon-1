...
Правила:
- Опирайся только на эти данные. Чего в них нет — прямо скажи «в данных этого нет» и не выдумывай.
- Главный источник о самом вузе — university_info: факты с официального сайта и из Википедии (спорт и секции, еда и столовые, общежития и их цена, клубы и мероприятия, факультеты и язык обучения, поступление, стоимость, стипендии, иностранным студентам, кампус, рейтинги). Используй их активно и конкретно: названия, числа, цены, часы работы. around_campus — что рядом на карте (OpenStreetMap и Google), climate — погода за год.
...
- Когда человек пишет о своих предпочтениях или задаёт вопрос, не повторяй вступление. Отвечай по его запросу: «### Что вам подойдёт в <name A>», «### Что вам подойдёт в <name B>» (конкретно, с фактами), затем «### Что может не подойти» и, если нужно, «### Что стоит уточнить» (что спросить у приёмной комиссии). Пробелы в данных — одной строкой в конце. Короткий вопрос — короткий ответ."""
...
async def advisor_sheet(p: Profile, lang: str = "ru") -> dict:
    ...
    from . import site_digest
    try:
        d = await site_digest.digest(u, wait=45)
    except Exception as e:  # noqa: BLE001
        log.info("advisor site digest %s: %r", u.qid, e)
        d = None
    if d and d.get("topics"):
        sheet["university_info"] = {site_digest.TOPICS[k][0]: [f["text"] for f in facts]
                                    for k, facts in d["topics"].items() if k in site_digest.TOPICS}
        if d.get("summary"):
            sheet["university_summary"] = d["summary"]
