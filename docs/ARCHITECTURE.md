# Архитектура CampusLense

Рабочее название продукта: **CampusLense** — «покажи университет таким, каким его увидит студент».

## 0. Что изменилось 17.09 (актуальнее разделов ниже)

- **ИИ-инспектор** (`pipeline/ai_inspector.py`): Gemini 3.5 Flash-Lite в низком разрешении смотрит *каждое* фото рядом с
  эталонным снимком вуза (Wikidata P18) и возвращает: место (этот вуз / город / другое место / не фото), уверенность 0–3,
  категорию, наглядность 0–3, флаги (сток, баннер, фрагмент, рисунок, официальная встреча…), десятилетие. Пачки по 12,
  3 параллельно, ответы кэшируются по (вуз, SHA-1). Заменил старого «судью» спорных фото.
- **Уверенность** (`pipeline/verify.py`): источник сам по себе не доказательство. Вердикт ИИ даёт от −0.6 до +0.30;
  фото из статьи о городе остаётся фото города, пока в подписи нет названия вуза; радиус 500 м весит меньше полигона OSM;
  сходство с эталоном по CLIP ≥ 0.82 даёт +0.10. Пороги: подтверждено ≥ 0.65, вероятно ≥ 0.40.
- **Новые источники**: Google Картинки по категориям (Serper), Instagram и TikTok вуза и поиск TikTok (ScrapeCreators),
  ролики о вузе с любых каналов (YouTube Data API), кадры из середины роликов канала вуза, поиск Commons по названию,
  Openverse. Поисковая выдача — только кандидаты: без вердикта ИИ не показывается; без названия вуза на странице нужна
  уверенность 3/3.
- **Куратор** (`pipeline/curate.py`): 6–8 лучших и непохожих кадров на категорию, остальное — «Показать все».
- **Время**: загрузки останавливаются к сроку сбора (бюджет − 6 с) и сохраняют полученное, жёсткий потолок 29 с.
- **Интерфейс**: ссылка на источник под каждым фото, блок «Что увидел ИИ-инспектор» в паспорте, статистика инспектора
  и эталон в режиме жюри.
- **Метрика** (`scripts/eval.py`, 7 вузов, ~200 фото): precision 0.94, recall 0.94, категории 0.96.
